import { dispositivoSeguranca, administrador, mensagem, paragrafo} from "./script.js";
export function cadastrarDispositivoSeguranca() {
...
export function retornarTodosDispositivoSeguraca() {
    administrador.innerHTML = ''
    paragrafo.innerHTML = ''
    mensagem.innerHTML = ''
    let dispositivosArray = Object.values(dispositivoSeguranca);
    let listaFormatada = dispositivosArray.map(function(dispositivo) {
        return `Nome Dispositivo: ${dispositivo.nomeDispositivo}, Situação do Dispositivo: ${dispositivo.situacao}, Disponibilidade do Dispositivo: ${dispositivo.disponibilidade}`;
    }).join('<br>');
    // document.getElementById('dispositivos-lista').innerHTML = listaFormatada;
    paragrafo.innerHTML = listaFormatada
}

export function pesquisarDispositivoSeguranca(){
    administrador.innerHTML = ''
    paragrafo.innerHTML = ''
    mensagem.innerHTML = ''
...
export function excluirDispositivoSeguranca() {
    administrador.innerHTML = ''
    paragrafo.innerHTML = ''
    mensagem.innerHTML = ''
    const entrada1 = document.createElement('input')
